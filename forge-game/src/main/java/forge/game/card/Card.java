...
public class Card extends GameEntity implements Comparable<Card>, IHasSVars, ITranslatable {
            ...
            try {
                if (keyword.startsWith("etbCounter")) {
                    ...
                    sbLong.append(inst.getReminderText()).append("\r\n");
                        ...
                        || keyword.startsWith("Graft") || keyword.startsWith("Fading") || keyword.startsWith("Durability:") || keyword.startsWith("Vanishing:")
    ...
    public final FCollectionView<SpellAbility> getBasicSpells(final CardState state) {
        final FCollection<SpellAbility> result = new FCollection<>();
        for (final SpellAbility ability : state.getNonManaAbilities()) {
            if (ability.isSpell() && ability.isBasicSpell()) {
                result.add(ability);
            }
        }
        return result;
