...
- 炉石模式使用 30 点初始生命、至少 30 张主牌、7 张起手、10 张手牌上限与递增疲劳；每个维持阶段选择一种基本地加入手牌。生物的已标记伤害跨 cleanup 保留，且每个生物都自动具有 `CanBeAttacked` 的攻击目标资格，可被对手生物直接选择为攻击目标；这不是强制阻挡，也不改变飞行、马术、威慑等异能原本的结算。
