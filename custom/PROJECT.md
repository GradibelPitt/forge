...
- `Mystery`（奥秘）已实现为共用背面的引擎关键词：真实卡面始终是 `Enchantment Mystery` 永久物牌，手牌中正常显示且可被展示；施放时只能支付 `{1}{U}{U}` 牌面朝下施放，任何方式进入战场也统一牌面朝下。`蓝色奥秘` 本身是 `TOKEN_HS` #1 的真实 `结界～奥秘` 卡牌，Oracle 为 `你的对手隐藏了一些秘密。`；其他奥秘的战场牌框对操控者和对手都始终映射到该卡面，但底层保留各自真实正面的全部效果。操控者悬停／查看详情时可看到真实正面，对手不可见；仅可在对手回合免费翻回正面，随后触发牺牲自身并结算每张牌的 `MysteryEffect`。
