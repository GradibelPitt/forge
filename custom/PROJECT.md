...
- 玩家级 `GrantSpellRule` 已接入真实费用与法术力转换热路径；规则在本局内独立于来源牌持续存在，支持稳定键幂等或显式叠加。其独立 `Harmony` 模式按牌的拥有者向游戏内卡牌与 `CardView` 投射可见“调和”关键字，并以专用减费语义先减有色符号、再减非 X 通用费用；不改变旧 `ReduceGeneric`。公开牌只在标签布尔值变化时重建，隐藏牌库使用 epoch 惰性同步。“弃暗投明”已迁移到该模式，卡面描述不变。
