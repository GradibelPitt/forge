...
- `Mystery`（奥秘）已实现为共用背面的引擎关键词：真实卡面始终是 `Enchantment Mystery` 永久物牌，手牌中正常显示且可被展示；施放时只能支付 `{1}{U}{U}` 牌面朝下施放，任何方式进入战场也统一牌面朝下。公共背面为蓝色 `蓝色奥秘`、`结界～奥秘` 和固定提示文字；仅可在对手回合免费翻回正面，随后触发牺牲自身并结算每张牌的 `MysteryEffect`。
