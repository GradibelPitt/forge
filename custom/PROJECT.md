...
- `Mystery`（奥秘）已实现为共用背面的引擎关键词：真实卡面始终是 `Enchantment Mystery` 永久物牌，手牌中正常显示且可被展示；施放时只能支付 `{1}{U}{U}` 牌面朝下施放，任何方式进入战场也统一牌面朝下。`蓝色奥秘` 本身是 `TOKEN_HS` #8 的真实 `结界～奥秘` 卡牌，Oracle 为 `你的对手隐藏了一些秘密。`；其他奥秘的战场牌框对操控者和对手都始终映射到该卡面，但底层保留各自真实正面的全部效果。操控者悬停／查看详情时可看到真实正面，对手不可见；仅可在对手回合且 `MysteryEffect` 的全部强制目标都有合法候选时免费翻回正面。翻面触发先正常结算；该触发离开堆叠后，正面奥秘才在下一次状态检查中退场，避免效果读取来源前被移走，也不会在触发结算、失效或被反击后留在战场。
...
- `Windfury`（风怒）已作为共享额外战斗关键词接入 `PhaseHandler`：主动牌手的第一次战斗结束后，只要操控至少一个风怒生物，就共同获得恰好一次额外战斗；开始该战斗时重置其风怒生物，且只有风怒生物可攻击。
- 构筑大厅以独立“炉石传说”模式替代原“魔王／魔王混斗”复选框；模式位于“时空竞逐”之后。规则完全由引擎持有，不再登记或加载同名神器、徽记或指挥区对象。
- 炉石模式使用 30 点初始生命、至少 30 张主牌、7 张起手、10 张手牌上限与递增疲劳；每个维持阶段选择一种基本地加入手牌。生物的已标记伤害跨 cleanup 保留，且每个生物都自动具有 `CanBeAttacked` 的攻击目标资格。攻击生物只有在按普通成对阻挡限制能够阻挡目标生物时才能攻击它，例如飞行可被飞行或延势攻击、马术可被马术攻击；这不会建立强制阻挡关系。
- 通用 `GameRule` 规则牌保护机制仍保留给其他未来规则牌：在起手与调度之前放逐并锁定在放逐区，同时从发现、幻变／制造、批量数据库替换、选牌、直接打出和复制中排除。
...
- 启迪者伊利斯通过 `Count$StartingDeckDuplicateNonlandNames` 检查注册的起始主牌构筑；地牌不参与重名判定，同名不同版本会合并计数，满足条件时在施放触发结算时幻变当前手牌中每张牌的一个复制。
