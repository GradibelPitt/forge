...
| 脱困古神尤格萨隆 | `{15}`，7/5 传奇生物～古神 | `cards/colorless/脱困古神尤格萨隆.txt` | 36 | 恐惧、敏捷；每个本局已释放的非生物咒语使其费用减少 `{1}`；每次竭绝均造一个“混乱触须”神器衍生物（横置并牺牲后，发现法术力值为坟墓场触须数 + 1 的法术并对随机目标免费施放）；三项竭绝分别永久夺取非神器生物、煽惑所有敌方非神器生物，以及按坟墓场瞬间/法术牌数量随机化生并免费施放法术。 |
