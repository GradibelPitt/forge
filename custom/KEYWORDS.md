...
- **Tests:** `PlayerSpellRuleRegistryTest` 覆盖玩家隔离、稳定键幂等、显式叠加、无承载牌、颜色支付、旧通用减费兼容、调和先减有色符号、可见关键字、无色排除、清理与复制生命周期。
