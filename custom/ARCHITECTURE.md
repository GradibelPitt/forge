...
- `forge-game/src/main/java/forge/game/card/CardFactoryUtil.java`、`card/Card.java`、`spellability/AbilityStatic.java`、`spellability/SpellAbilityRestriction.java`、`GameAction.java` 与 `ability/effects/SetStateEffect.java`：奥秘的 `{1}{U}{U}` 隐藏施放、共用 `蓝色奥秘` 背面、正面施放禁令、任意进战场自动背面、仅对手回合且强制目标合法时免费翻面，以及 `MysteryEffect` 触发离开堆叠后的状态检查退场。`蓝色奥秘` 是 `TOKEN_HS` #8 的真实卡牌；`forge-core/.../ImageKeys.java` 将其他奥秘的背面统一映射为其收藏编号卡牌键 `c:蓝色奥秘|TOKEN_HS|[8]`，并由 `cards/pictures/TOKEN_HS/蓝色奥秘.artcrop.jpg` 动态绘制完整卡面。`forge-gui-desktop/.../CardPanel.java` 与 `forge/CachedCardImage.java` 保证牌面朝下时战场牌框固定使用这一背面键，而操控者悬停详情仍可读取保留全部效果的真实正面；`forge-gui/res/lists/TypeLists.txt` 登记 `Mystery` 结界子类别。
