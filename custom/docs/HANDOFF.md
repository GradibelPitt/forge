...
- 准备开房的玩家应使用普通入口更新到当前运行包；`forge-gui` 的 UPnP/IGD v2 overlay 用于提高房主自动建立路由器端口映射的兼容性。
- 该 UPnP 修复不改变联机协议、序列化事件或加入者行为，因此仅加入别人房间的玩家无需只为此修复更新。
- 如果同一运行包还发布了 DIY 卡牌、版本表、卡图或本地化资源，实际使用这些内容联机的参与者仍应更新，以避免资源不一致。
- 普通更新会自动拉取 Git payload、校验 manifest 并同步 profile；只有普通入口失败时才使用强制修复入口。
