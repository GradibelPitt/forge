...
替换完成后通过 `GrantSpellRule` 把规则直接登记到施法者的对局状态，不创建永久物或指挥区徽记。该规则直接匹配 `Card.nonColorless`：你的所有有色咒语获得 `ManaConversion$ AnyType->AnyColor`，并减少 `{2}` 通用费用。费用计算与实际支付分别从玩家注册表读取；注册表只按规则数线性遍历，不访问全局卡牌数据库或任何区域。同一稳定键重复登记为幂等操作，避免多次结算产生不断增长的重复记录。用户给定的卡面描述保持不变。
