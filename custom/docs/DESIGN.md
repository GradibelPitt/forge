...
“弃暗投明”使用独立 `ReplaceCards` API 一次性处理手牌与牌库。替换过程对每个区域做一次线性计划扫描并记录符合条件牌的稳定位置，再从缓存的同法术力值候选桶中随机选择；原牌直接停止存在，新牌通过标准换区路径进入原位置，未找到候选桶的原牌保持不变。移动被替代或阻止时不登记替换名称；成功名称先在本次结算内去重，所有区域完成后才一次性合并。候选缓存只保存 `PaperCard`，避免循环调用发现、重复扫描全数据库、逐牌 `indexOf`、额外位置定位扫描或批量实例化游戏对象；有序手牌另做身份成员校验，Forge 有序区底层列表的标准移除／插入本身仍可能是 O(n)，因此不把整个执行宣称为严格 O(k)。`MatchManaValue$ True` 是显式必填契约，缺失或 `False` 会在任何副作用前拒绝。
